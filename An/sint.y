...
%token NUMERO REP CM_RUTA CM_NOMBRE VD_ID_NUMERO EXTENSION_DSK B E L  RF BF FF WF K M CHGRP GRP PAUSE COMANDO_ID R CHOWN CP DEST FIND CAT MV RM REN P MKFILE MKDIR LOGOUT ID  FILE_N EDIT MKGRP RMGRP USR  MOUNT RMDISK FLECHA PATH ADD   EXEC RUTA MKDISK SIZE NAME UNIT FDISK TYPE FIT DELETE fast full UNMOUNT MKFS  PWD RMUSR MKURS CHMOD UGO CONT
%type <str>NUMERO CM_RUTA CM_NOMBRE REP VD_ID_NUMERO EXTENSION_DSK B E L  RF BF K M FF WF CHGRP GRP PAUSE COMANDO_ID R CHOWN CP DEST FIND CAT MV RM REN P MKFILE MKDIR LOGOUT ID FILE_N EDIT MKGRP RMGRP USR MOUNT RMDISK FLECHA PATH ADD    EXEC RUTA MKDISK SIZE  NAME UNIT FDISK TYPE FIT DELETE fast full UNMOUNT MKFS  PWD RMUSR MKURS CHMOD UGO CONT
// producciones o no terminales 
%type <NoTerminal>PARAMETROS_REPORTES P_FDISK REPORTES INICIO OPCIONES_FIT MONTAR CADENA_O_ID  MENU_COMANDOS CREAR_DISCO TAM ELIMINAR_DISCO PARAMETROS_MKDISK P_MKDISK ADMINISTRAR_PARTICIONES TAM2 TYPE_PARTICION OPCIONES_DELETE P_REPORTE
...
PARAMETROS_REPORTES : PARAMETROS_REPORTES P_REPORTE
                    | P_REPORTE
					;
P_REPORTE: '-' CM_NOMBRE FLECHA  ID { Name_ = $4}
